...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/upload", upload.single('file'), async (req, res) => {
    try {
      ...
      // Create note content with file information
      let noteContent = content || '';
      
      // For images, we'll store the file as base64 in the note content
      if (file.mimetype.startsWith('image/')) {
        const base64Image = file.buffer.toString('base64');
        noteContent = `![${file.originalname}](data:${file.mimetype};base64,${base64Image})\n\n${noteContent}`;
      } else {
        // For other files, store file info and content description
        noteContent = `**File:** ${file.originalname}\n**Type:** ${file.mimetype}\n**Size:** ${(file.size / 1024 / 1024).toFixed(2)} MB\n\n${noteContent}`;
      }
      ...
      // Analyze the uploaded file with AI if it's text-based or an image
      if (file.mimetype.startsWith('image/') || file.mimetype.includes('text') || file.mimetype.includes('pdf')) {
        // Analyze with OpenAI first
        analyzeWithOpenAI(noteContent, 'detailed')
          .then(async (analysis) => {
            console.log("OpenAI analysis completed for uploaded file:", note.id);
          ...
          .catch(error => {
            console.error("OpenAI analysis failed for uploaded file:", note.id, error);
          });
      }
